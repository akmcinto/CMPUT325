...
#|
 ...
 in x are maintained in the output.  The flattening is done by testing if the (car x) is
 an atom.  If it is it is added to the front of the list, and the rest of the list
 (cdr x) is then flattened.  If (car x) is not an atom, then append is used to add it
 to the list.  By the properties of append, each time append is called it
 removes one layer of nesting on the non-atomic element.  Once the element is an atom,
 it is added into the flattened list.
...
#|
 ...
 the remaining elements of the longer list are appended to the output list.  The
 mixing is done by constructing a list from the first element of L1 and a recursive
 call to mix with the order of the list arguments reversed (i.e. (mix L2 L1)).  This
 will cause the order of the elements added to the output list to alternate.  Once either
 list becomes empty the rest of the other list is returned to be added to the end of
 the output.
...
#|
 ...
 are then added to a and b alternatingly, by giving the first element of L to a, the
 second element to b, and then recursively calling accumlate_splits on the remainder
 of L.
|#
